...
@Service
public class HotelRoomDataInboundServiceImpl implements HotelRoomDataInboundService {

  private static final Logger LOGGER = LoggerFactory
      .getLogger(HotelRoomDataInboundServiceImpl.class);
  ...
  @Override
  @KafkaListener(topics = KafkaTopic.HOTEL_ROOM)
  public void createHotelRoomData(String message) throws Exception {
    ...
    LOGGER.info("createHotelRoomData request message {}, mandatoryRequest {}", message,
        mandatoryRequest);
    List<VariableSource> variableSources = this.mapDataToHotelRoom(message, mandatoryRequest);

    this.variableSourceService.updateVariableSources(mandatoryRequest, variableSources).subscribe();
    LOGGER.info("createHotelRoomData response variableSources {}", variableSources);
  }

  private List<VariableSource> mapDataToHotelRoom(String message, MandatoryRequest
    ...
    for (Map<String, String> data : JSONHelper.convertJsonInStringToObject(message,
      ...
      variableSource.setSourceType(VariableSourceTable.HOTEL_ROOM);
      variableSource.setValueId(data.get("roomId"));
      variableSource.setValueName(data.get("roomName"));
      variableSource.setValueSearch(data.get("roomName").toLowerCase());
