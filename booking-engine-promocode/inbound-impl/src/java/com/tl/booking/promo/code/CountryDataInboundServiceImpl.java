...
@Service
public class CountryDataInboundServiceImpl implements CountryDataInboundService {

  private static final Logger LOGGER = LoggerFactory.getLogger(CountryDataInboundServiceImpl.class);
  ...
  @Override
  @KafkaListener(topics = KafkaTopic.COUNTRY)
  public void createCountryData(String message) throws Exception {
    ...
    LOGGER.info("createCountryData request message {}, mandatoryRequest {}", message,
        mandatoryRequest);
    List<VariableSource> variableSources = this.mapDataToCountry(message, mandatoryRequest);

    this.variableSourceService.updateVariableSources(mandatoryRequest, variableSources).subscribe();
    LOGGER.info("createCountryData response variableSources {}", variableSources);
  }

  private List<VariableSource> mapDataToCountry(String message, MandatoryRequest
    ...
    for (Map<String, String> data : JSONHelper.convertJsonInStringToObject(message,
      ...
      variableSource.setSourceType(VariableSourceTable.COUNTRY);
      variableSource.setValueId(data.get("countryId"));
      variableSource.setValueName(data.get("countryName"));
      variableSource.setValueSearch(data.get("countryName").toLowerCase());
