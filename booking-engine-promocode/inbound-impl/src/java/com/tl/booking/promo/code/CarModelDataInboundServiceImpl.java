...
@Service
public class CarModelDataInboundServiceImpl implements CarModelDataInboundService {

  private static final Logger LOGGER = LoggerFactory
      .getLogger(CarModelDataInboundServiceImpl.class);
  ...
  @Override
  @KafkaListener(topics = KafkaTopic.CAR_MODEL)
  public void createCarModelData(String message) throws Exception {
    ...
    LOGGER.info("createCarModelData request message {}, mandatoryRequest {}", message,
        mandatoryRequest);
    List<VariableSource> variableSources = this.mapDataToProvince(message, mandatoryRequest);

    this.variableSourceService.updateVariableSources(mandatoryRequest, variableSources).subscribe();
    LOGGER.info("createCarModelData response variableSources {}", variableSources);
  }

  private List<VariableSource> mapDataToProvince(String message, MandatoryRequest
    ...
    for (Map<String, String> data : JSONHelper.convertJsonInStringToObject(message,
      ...
      variableSource.setSourceType(VariableSourceTable.CAR_MODEL);
      variableSource.setValueId(data.get("provinceId"));
      variableSource.setValueName(data.get("provinceName"));
      variableSource.setValueSearch(data.get("provinceName").toLowerCase());
