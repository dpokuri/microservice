...
@Service
public class CarVendorDataInboundServiceImpl implements CarVendorDataInboundService {

  private static final Logger LOGGER = LoggerFactory
      .getLogger(CarVendorDataInboundServiceImpl.class);
  ...
  @Override
  @KafkaListener(topics = KafkaTopic.CAR_VENDOR)
  public void createCarVendorData(String message) throws Exception {
    ...
    LOGGER.info("createCarVendorData request message {}, mandatoryRequest {}", message,
        mandatoryRequest);
    List<VariableSource> variableSources = this.mapDataToCarVendor(message, mandatoryRequest);

    this.variableSourceService.updateVariableSources(mandatoryRequest, variableSources).subscribe();
    LOGGER.info("createCarVendorData response variableSources {}", variableSources);
  }

  private List<VariableSource> mapDataToCarVendor(String message, MandatoryRequest
    ...
    for (Map<String, String> data : JSONHelper.convertJsonInStringToObject(message,
      ...
      variableSource.setSourceType(VariableSourceTable.CAR_VENDOR);
      variableSource.setValueId(data.get("masterId"));
      variableSource.setValueName(data.get("vendorName"));
      variableSource.setValueSearch(data.get("vendorName").toLowerCase());
