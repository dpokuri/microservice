...
@Service
public class HotelDataInboundServiceImpl implements HotelDataInboundService {

  private static final Logger LOGGER = LoggerFactory.getLogger(HotelDataInboundServiceImpl.class);
  ...
  @Override
  @KafkaListener(topics = KafkaTopic.HOTEL)
  public void createHotelData(String message) throws Exception {
    ...
    LOGGER.info("createHotelData request message {}, mandatoryRequest {}", message,
        mandatoryRequest);
    List<VariableSource> variableSources = this.mapDataToHotel(message, mandatoryRequest);

    this.variableSourceService.updateVariableSources(mandatoryRequest, variableSources).subscribe();
    LOGGER.info("createHotelData response variableSources {}", variableSources);
  }

  private List<VariableSource> mapDataToHotel(String message, MandatoryRequest
    ...
    for (Map<String, String> data : JSONHelper.convertJsonInStringToObject(message,
      ...
      variableSource.setSourceType(VariableSourceTable.HOTEL);
